  ...
  async execute(interaction) {
    ...
    const createEmbed = (page) => {
      ...
      const embed = new EmbedBuilder()
        .setColor(0x00ff00)
        .setTitle(
          `Recent Sales (${filter === "all" ? "All Time" : filter === "today" ? "Today" : "This Week"})`,
        )
        .setDescription(
          `Showing ${sales.length} most recent transaction${sales.length > 1 ? "s" : ""}`,
        )
        .setTimestamp();
      ...
      const totalCount = salesTracker.db
        ...
        .get().count;
      embed.setFooter({
        text: `Page ${page + 1}/${totalPages} • Total tracked transactions: ${totalCount}`,
      });
